(function attachClozeViewModelHelpers(global) {
  function createInitialClozeAnswerState(items) {
    if (!Array.isArray(items) || items.length === 0) return [];
    return items.map(() => ({ checked: false, correct: false, userAnswer: '' }));
  }

  function getClozeCardStateViewModel(state) {
  ...
  function buildClozeCardViewModel(item, state, index) {
  ...
  function buildClozeQuizViewModel(items, answerState) {
  ...
  global.ClozeViewModelHelpers = {
    createInitialClozeAnswerState,
    getClozeCardStateViewModel,
    buildClozeCardViewModel,
