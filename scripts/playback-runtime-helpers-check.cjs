...
const assemblySource = fs.readFileSync(path.join(repoRoot, 'src', 'composables', 'reader-runtime-assembly.js'), 'utf8');
  ...
  assert.ok(
    ...
    'reader-runtime should delegate playback helpers through reader-runtime-assembly'
  );
  assert.ok(
    assemblySource.includes("import { initReaderFeatureRuntime } from './reader-feature-runtime.js';"),
    'reader-runtime-assembly should initialize playback helpers through reader-feature-runtime'
  );
  assert.ok(
