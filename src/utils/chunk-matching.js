...
export function findExactMatchRange(wordList, phraseTokens, fromIndex = 0) {
...
export function findExactMatch(wordList, phraseTokens, fromIndex = 0) {
    return findExactMatchRange(wordList, phraseTokens, fromIndex);
}

export function adjustIndex(baseIdx, targetWord, wordList, searchRange) {
...
export function scoreMatchCandidate(firstWord, lastWord, startWord, endWord) {
    return (firstWord && startWord === firstWord ? 1 : 0)
        + (lastWord && endWord === lastWord ? 1 : 0);
}

export function normalizeChunkCandidateBounds(rawStart, rawEnd) {
    const start = Number.isFinite(rawStart) ? rawStart : 0;
    const end = Number.isFinite(rawEnd) ? rawEnd : start;
    return { start, end };
}

export function buildChunkCandidateVariants(rawStart, rawEnd) {
...
export function buildChunkMatchWindow(startIndex, endIndex, phraseTokensLength) {
...
export function clampChunkMatchCandidate(candidate, segmentLength) {
...
export function buildChunkCandidateEndWindow(startIndex, endIndex, phraseTokensLength, segmentLength) {
...
export function getChunkCandidateBoundaryWords(wordList, startIndex, endIndex) {
    return {
...
export function normalizeChunkMatchCandidate(startIndex, endIndex, score) {
    return {
        st: startIndex,
        ed: endIndex,
        score
